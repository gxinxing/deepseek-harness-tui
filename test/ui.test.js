...
describe('splitCodeBlocks', () => {
  ...
  test('trailing newline after closing fence produces trailing empty text block', () => {
    // ui.js line-based splitCodeBlocks preserves trailing newlines as an
    // empty text block; markdownLines handles this gracefully by emitting
    // one empty Text element (no crash, no double-render).
    const input = 'before\n```\ncode\n```\n';
    const result = splitCodeBlocks(input);
